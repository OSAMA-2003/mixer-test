...
import { fetchSpecialItems } from "@/lib/api";
...
export default function MixerFeatured({ activeCategory, onAddToCart }: MixerFeaturedProps) {
  ...
  const [dishesList, setDishesList] = useState<MenuItem[]>(DISHES);
  const { addToCart } = useCart();

  useEffect(() => {
    async function loadSpecial() {
      const special = await fetchSpecialItems();
      if (special && special.length > 0) {
        const formatted = special.map((s) => ({
          id: s.id,
          name: s.name,
          category: (s.category_id as any) || "juices",
          price: s.price,
          badge: s.badge || "سبيشيال ✨",
          img: s.image || s.img || "/products/p1.jpg",
          desc: s.description,
        }));
